...
# draw obj model with square-grid by drag-drop the obj file on window
...
def main():
    ...
    vao_grid_wh = prepare_vao_grid_wh()
    vao_grid_bk = prepare_vao_grid_bk()
    ...
    while not glfwWindowShouldClose(window):
        ...
        for i in range(-20, 20):
            for j in range(-10, 10):
                if (i % 2):
                    M = glm.translate((i, -1, j * 2))
                    draw_grid(shader_program, vao_grid_wh, P * V * M)

                    M = glm.translate((i, -1, j * 2 + 1))
                    draw_grid(shader_program, vao_grid_bk, P * V * M)
                else:
                    M = glm.translate((i, -1, 2 * j))
                    draw_grid(shader_program, vao_grid_bk, P * V * M)

                    M = glm.translate((i, -1, 2 * j + 1))
                    draw_grid(shader_program, vao_grid_wh, P * V * M)
