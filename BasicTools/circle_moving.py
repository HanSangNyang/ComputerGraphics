...
# move a circle by moving cursor
...
def cursor_callback(window):
    ...
    return glm.translate(((pos.x - w_width/2) * .0025, -((pos.y - w_height/2))*.0025, 0.0))

def prepare_vao_circle():
    global circle_color

    circle_dot = [0.0, 0.0, 0.0] # center of circle
    circle_dot.extend((1.0,1.0,1.0))
...
def main():
    ...
    window = glfwCreateWindow(w_width, w_height, 'circle-moving', None, None)
    if not window:
        glfwTerminate()
        return
    ...
    while not glfwWindowShouldClose(window):
        ...
        draw_circle(shader_program, vao_circle, M)
